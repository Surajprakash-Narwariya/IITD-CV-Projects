...
def run():
    ...
    for DELAY in delay_list:
        for i in range(0, NUM_ITERATIONS):
            ...
            pref_c1 = "1"
            pref_c2 = "2"
            s1_cmd = f"python3 p2_server.py {SERVER_IP1} {SERVER_PORT1} > server1.log 2>&1 &"
            s2_cmd = f"python3 p2_server.py {SERVER_IP2} {SERVER_PORT2} > server2.log 2>&1 &"
            c1_cmd = f"python3 p2_client.py {SERVER_IP1} {SERVER_PORT1} --pref_outfile {pref_c1} > client1.log 2>&1 &"
            c2_cmd = f"python3 p2_client.py {SERVER_IP2} {SERVER_PORT2} --pref_outfile {pref_c2} > client2.log 2>&1 &"
            ...
            while end_time_c1 is None or end_time_c2 is None:
                ...
                if end_time_c1 is None:
                    ...
                    if not result_c1 or str(c1_pid) not in result_c1:  # Process is no longer running
                        ...
                        if os.path.exists(f"/proc/{c1_pid}"):  # .pid_exists(int(c1_pid)):
                            os.kill(int(c1_pid), signal.SIGTERM)
                            print(f"Killed client 1 with PID: {c1_pid}")
                        else:
                            print(f"Process with PID {c1_pid} does not exist.")
                ...
                if end_time_c2 is None:
                    ...
                    if not result_c2 or str(c2_pid) not in result_c2:  # Process is no longer running
                        ...
                        # if os.path.exists(f"/proc/{c2_pid}"):
                        #     os.kill(int(c2_pid), signal.SIGTERM)
                        #     print(f"Killed client 1 with PID: {c2_pid}")
                        # else:
                        #     print(f"Process with PID {c2_pid} does not exist.")
