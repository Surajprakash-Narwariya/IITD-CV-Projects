...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if(parsed_cmd->type == EXEC) {
    	struct execcmd *ecmd = (struct execcmd*)parsed_cmd;
    	int argc = 0;
    	while (ecmd->argv[argc] != 0) {
      	argc++;
    	}
    	//char **argv = ecmd->argv;
    	if(argc > 0){
	    	char *cmd = ecmd->argv[0];
	  		//printf(1, "%d %s %d\n",argc, cmd, atoi(argv[1]));
	    ...
	    if(strcmp(cmd, "block") == 0){
		    ...
		    if(argc < 2){
  	  		...
  	  		if (unblock(syscall_id) < 0) {
  	    		printf(2, "Failed to unblock syscall %d\n", syscall_id);
  	   			...
  	   			printf(1, "syscall %d is unblocked\n", syscall_id);
  	  		}
	  		}	
  			continue;
			}
			...
			if (strcmp(cmd, "chmod") == 0) {
  	    	...
  	    	//	mode = strtol(argv[2], 0, 8); // Parse octal (e.g., "0755")
  	  		//} else {
  	  		
  	  		trim(ecmd->argv[1]);  // Remove trailing newline/whitespace from filename.
			    trim(ecmd->argv[2]);
  	    	mode = atoi(ecmd->argv[2]);
  	  		//}
  	  		if (chmod(ecmd->argv[1], mode) < 0) {
