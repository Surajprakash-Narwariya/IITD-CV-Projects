...
{  
  ...
  case T_PGFLT:
    ...
    if(!curproc->pgdir || va >= curproc->sz)
      goto kill;

    pte_t *pte = walkpgdir(curproc->pgdir, (void*)va, 0);

    if((pte && !(*pte & PTE_SWAP)) || !pte) {
      kill:
          curproc->killed = 1;
    }else if(pte && (*pte & PTE_SWAP)){
      handle_pgfault(curproc->pgdir, va);
    }
    break;
  ...
  int get_no_pages(struct proc *p){
    int pgs = PGSIZE;
    int num_pages = (p->sz + pgs - 1) / pgs;
    return num_pages;
  }
