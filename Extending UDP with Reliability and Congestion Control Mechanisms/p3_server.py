...
cubicWindow = INITIAL_CWND
C = 0.4
BETA = 0.5
Wmax = 64
lastCongestionWindow = Wmax
packetLostTime = time.time()
...
def send_file(server_ip, server_port, enable_fast_recovery):
    ...
    global cubicWindow, C, BETA, Wmax, packetLostTime, lastCongestionWindow
    ...
    with open(FILE_PATH, 'rb') as file:
        ...
        TIMEOUT = max(EstimatedRTT + 4*devRTT, 0.1)
        ...
        while True:
            ...
            while seq_num < window_base + cubicWindow and seq_num < len(packetData):
                # chunk = file.read(MSS)
            ...
            try:
                ...
                while(n > 0):
                    ...
                    if ack_seq_num > last_ack_received:
                        ...
                        if(cubicWindow < ssthresh and window_increment_flag):
                            cubicWindow = 2*cubicWindow
                            window_increment_flag = False
                        elif(window_increment_flag):
                            lastCongestionWindow = cubicWindow
                            cubicWindow = math.floor(findCubicWindow(packetLostTime, Wmax))
                            print(cubicWindow)
                        ...
                        if enable_fast_recovery and duplicate_ack_count >= DUP_ACK_THRESHOLD:
                            ...
                            ssthresh = max(1, cubicWindow//2)
                            lastCongestionWindow = cubicWindow
                            cubicWindow = findCubicWindow(packetLostTime, Wmax)
                ...
                print(seq_num,ack_seq_num, window_base, cubicWindow, ssthresh)
                # retransmit_unacked_packets(server_socket, client_address, unacked_packets)
...
def findCubicWindow(packetLostTime,Wmax):
    K = calculateK()
    t = time.time() - packetLostTime
    cubicWindow = C * (t - K)**3 + Wmax
    Wmax = lastCongestionWindow
    packetLostTime = t
    return cubicWindow
