...
sys_signal(void)
{
  sighandler_t handler_function;
  if(argptr(0, (void*)&handler_function, sizeof(handler_function)) < 0)return -1;
  if(exists&1){
    myproc()->pending_custom_handler = 1&0;
    myproc()->custom_handler_function = handler_function;
  }else{
    return -1;
  }
  return 0;
