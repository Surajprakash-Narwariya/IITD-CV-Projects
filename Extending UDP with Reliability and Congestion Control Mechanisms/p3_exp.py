...
def run(expname):
    ...
    # Output file 
    output_file = f'reliability_{expname}_p3.csv'
            ...
            for FAST_RECOVERY in [True]:
                for i in range(0, NUM_ITERATIONS):
                    ...
                    try:
                        ...
                        h1.cmd(f"python3 -u p3_server.py {SERVER_IP} {SERVER_PORT} > server.log 2>&1 & ")
                        start_time = time.time()
                        result = h2.cmd(f"python3 -u p3_client.py {SERVER_IP} {SERVER_PORT} > client.log 2>&1")
                        end_time = time.time()
